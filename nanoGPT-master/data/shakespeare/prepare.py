...
input_file_path = os.path.join(os.path.dirname(__file__), 'input.txt')  # 定义数据集文件的路径
if not os.path.exists(input_file_path):  # 检查数据集文件是否存在
    ...
    # 如果数据集文件不存在，就从网上下载
    with open(input_file_path, 'w') as f:  # 打开文件，准备写入
        f.write(requests.get(data_url).text)  # 从网上下载数据，并写入文件

# 读取数据集文件
with open(input_file_path, 'r') as f:  # 打开文件，准备读取
    data = f.read()  # 读取文件内容
n = len(data)  # 计算数据的长度
# 将数据集分为训练集和验证集
