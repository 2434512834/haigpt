...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_fc = nn.Linear(config.n_embd, 4 * config.n_embd,
                              bias=config.bias)  # 定义一个线性层，输入维度为config.n_embd，输出维度为4 * config.n_embd，是否添加偏置由config.bias决定
        self.gelu = nn.GELU()  # 定义一个GELU激活函数
        self.c_proj = nn.Linear(4 * config.n_embd, config.n_embd,
                                bias=config.bias)  # 定义一个线性层，输入维度为4 * config.n_embd，输出维度为config.n_embd，是否添加偏置由config.bias决定
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ln_1 = LayerNorm(config.n_embd,
                              bias=config.bias)  # 定义一个层归一化（LayerNorm）层，输入维度为config.n_embd，是否添加偏置由config.bias决定
