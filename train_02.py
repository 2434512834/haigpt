...
def get_argument_parser():
    ...
    parser.add_argument("--checkpoint_saving_interval", type=int, default=10,
                        help="多少 epoch 保存一次检查点")
    parser.add_argument("--num_workers", type=int, default=16,
                        help="用于数据加载的 CPU 核心数")
    return parser
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if epoch % args.checkpoint_saving_interval == 0 or epoch == args.checkpoint_saving_interval - 1:
            logger.info(f"保存检查点: Epoch {epoch}")
            model_engine.save_checkpoint(f"{args.checkpoint_saving_path}", tag=f"Epoch-{epoch}")
        epoch += 1
        val_loss = evaluate(train_loader, model_engine)
        log_epoch_info(epoch, val_loss)
