...
# 在训练循环中使用 get_data 方法加载数据
def train(data_engine, model_engine, optimizer, scheduler, epoch):
    model_engine.train()
    total_loss = 0.0
    with tqdm(total=len(data_engine)) as progress_bar:
        for idx in range(len(data_engine)):
            data = data_engine.get_data(idx).cuda(non_blocking=True)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        train(data_engine, model_engine, optimizer, scheduler, epoch)
        epoch += 1
        if epoch % 1 == 0:
